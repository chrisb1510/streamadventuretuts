...
(function() {
  var cipher, crypt, ctype, parser, pw, tar, through, zipper, zlib;
  ...
  parser.on('entry', function(e) {
    ...
    thr = through(null, function() {
      return this.queue(" " + e.path + "\n");
    });
    return e.pipe(crypt.createHash('md5', {
      encoding: 'hex'
    })).pipe(thr).pipe(process.stdout);
  });

  zipper = zlib.createGunzip();

  process.stdin.pipe(cipher).pipe(zipper).pipe(parser);

}).call(this);

//# sourceMappingURL=secretz.js.map
